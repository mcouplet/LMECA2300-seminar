...
void script_csf() {
	...
	// SPH parameters
	int n_per_dim = 101; //101 number of particles per dimension
	...
	Particle_derivatives** particles_derivatives = (Particle_derivatives**) malloc(n_p * sizeof(Particle_derivatives*));
...
// 	double h = 2*l / (n_per_dim-1); // step between neighboring particles
	...
	Residual** residuals = (Residual**)malloc(N_tot * sizeof(Residual*));
...
void script1() {
	...
	Kernel kernel = Cubic;
	Setup* setup = Setup_new(50, timestep, kh,verlet, kernel);
