...
void* loop_with_drawing_for_mac(void* args) {
	...
	if (DRAWING) {
		...
		if (use_cells) {
			gridPoint = find_grid_points_for_mac(kh + L, 100);
...
void fillData_for_mac(GLfloat(*data)[14], GLfloat(*coord)[2], int nPoints)
{
	...
	for (int i = 0; i < 201; i++) {
        ...
        for(int j=0; j < 201; j++){
            data[201*i+j][0] = -100.0 + i;
            data[201*i+j][1] = -100.0 + j;
            coord[201*i+j][0] = data[201*i+j][0];
            coord[201*i+j][1] = data[201*i+j][1];
            float r = sqrt(data[201*i+j][0] * data[201*i+j][0] + data[201*i+j][1] * data[201*i+j][1]);
            data[201*i+j][8] = pow(data[201*i+j][0],3) + pow(data[201*i+j][1],3);
            data[201*i+j][9] = pow(data[201*i+j][1],3);
            //printf("(x,y)[%d] = ( %f, %f ) \n",201*i+j,data[201*i+j][8],data[201*i+j][9]);
            data[201*i+j][2] = 0; //speed x (not used by default visualization)
            data[201*i+j][3] = 0; // speed y (not used by default visualization)
            data[201*i+j][2] = rand() * 2.0 / RAND_MAX - 1.0; //Random starting speed
            data[201*i+j][3] = rand() * 2.0 / RAND_MAX - 1.0; //Random starting speed
            colormap_for_mac(r / rmax, &data[201*i+j][4]); // fill color
            data[201*i+j][7] = 0.8f; // transparency
        }
        // Defining the function for the moment the function is x^sin(x)
	}
}
...
int compute_optimal_verlet_for_mac(double timestep, double maxspeed, double kh) {
	...
	if (discri <= 0) {
		...
		else { if(ceil(max_ans>floor(max_ans)))
            return ceil(max_ans);
            else
...
neighborhood_for_mac** create_neighborhood_for_mac(GLfloat(*data)[14], GLfloat(*coord)[2], int iter, int data_filled, int nPoints, int use_cells, int use_improved_method, int radius_algorithm, int use_verlet, unsigned int seed)
{
	...
	while (iterations < iter) {
        ...
        int nNeigh = list_of_neighborhood[iterations][3].nNeighbours;
        nh[3].nNeighbours;
...
void show_equivalence_of_major_algorithm_combination_for_mac(int drawing) {
	...
	if (drawing) {
		...
		nhList3 = create_neighborhood_with_drawing_for_mac(data, coord, MAX_ITER, 0, NPTS, 1, 1, RADIUS_ALGORITHM, SEARCH_NEIGHBORHOOD, 0, seed);
		...
		nhList9 = create_neighborhood_for_mac(data, coord, MAX_ITER, 0, NPTS, 0, 1, RADIUS_ALGORITHM, 1, seed);
    ...
    for (int i = 0; i < NPTS; i++) {
        ...
        if (nNeigh > 0) {
            for (int j = 0; j < nNeigh; j++) {
                ...
                //printf("weight x = %f weight y = %f \n", weight_x, weight_y);
                val_div += -MASS / DENSITY * ((data[index_node2][8] - val_node_x) * weight_x + (data[index_node2][9] - val_node_y) * weight_y);
                //printf("val_div = %f \n", val_div );
                //printf("value div = %f \n",val_div);
                //printf("%f,%f,%f,%f,%f,%f,%f,%f \n",MASS,DENSITY,data[index_node2][8] , val_node_x , weight_x , data[index_node2][9] , val_node_y , weight_y);
                ...
                // aux = 2*((T_0(i)-T_0(j))) * factor * ( (-12./hsml(i)**2.) + (24.*r)/hsml(i)**3. - (12*r**2.)/hsml(i)**4. ) * (mass(j)/rho(j))
                //printf("value i = %f, value j = %f \n",val_node_x,data[index_node2][8]);
        ...
        data[i][13] = val_lapl;
        //printf("point de coordonnée %f %f div = %f \n", data[i][0], data[i][1], val_div);
    }

    FILE* fichier = NULL;
    
    fichier = fopen("new.txt", "w");
    
    for (int j = 0; j < NPTS; j++) {
        double exact = 3 * pow(data[j][1],2) + 3 * pow(data[j][0],2);
        double error = fabs(exact - data[j][10]);
        printf("exact value div = %0.9f \n", exact);
        printf("computed value div x= %0.9f \n", data[j][10]);
        printf("error = %0.9f \n \n", error);
        fprintf(fichier, "%0.9f ; %0.9f ; %f ; %f \n", error, data[j][10], data[j][0], data[j][1]);
    }
    fclose(fichier);
